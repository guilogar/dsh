...
public class Rope : MonoBehaviour 
{
	...
	private bool topeCuerda = true;
	
	public AudioSource inicio;
	public AudioSource inicio2;
	private bool gruasonando;
	private bool gruasonando2;
    ...
    //Rope data
    private float ropeSectionLength = 2.5f;
	...
	void Update() 
	{
		...
		//Controlar longitud de cuerda
		/*if (Input.GetKey(KeyCode.LeftControl))
		  ropeSectionLength += 0.01f;
	  
		if (Input.GetKey(KeyCode.LeftAlt))
		  ropeSectionLength -= 0.01f;*/
		if(ropeSectionLength < 1) {
		...
		if(topeCuerda) {
			var x = Input.GetAxis("Fire1") * Time.deltaTime * 0.4f;
			var y = Input.GetAxis("Fire2") * Time.deltaTime * 0.4f;
			ropeSectionLength += x - y;
			
			if(Input.GetButton("Fire1") && !gruasonando) {
				inicio.Play();
				gruasonando = true;
			}
			...
			if(Input.GetButton("Fire2") && !gruasonando2) {
				inicio2.Play();
				gruasonando2 = true;
			}
